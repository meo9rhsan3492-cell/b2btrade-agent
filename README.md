...
```
node src/index.js inquiry "John, ABC Corp, need 20 excavators"    # 询盘分类
node src/index.js reply --inquiry "John, ABC, need excavators"      # 生成邮件
node src/index.js followup --inquiry "John, ABC, need machine"     # 跟进计划
node src/index.js linkedin --search "mining equipment buyer"       # LinkedIn外展
node src/index.js pipeline "John, ABC Corp, need excavators"        # 一键串联
```

| Agent | 功能 | 特点 |
|-------|------|------|
| 🔍 询盘情报员 | 智能分类 | 高价值/真实客户/垃圾/竞品识别 |
| 📧 回复专家 | 邮件生成 | 个性化+卖点+CTA |
| 📆 跟进管家 | 跟进序列 | Day 1/3/7 自动触达 |
| 💼 LinkedIn外展 | 精准外展 | 决策者搜索+连接请求 |
| 🚀 Pipeline | 工作流编排 | 分类→邮件→跟进一键完成 |
